...
int main (int argc, char *argv[]) {	
	...
	*/
	...
	//TESTING
	FILE * finalTest = fopen("S-BoxTest.txt", "w");
	for(int i = 0; i < sBoxIndex; i++) {
		fputSbox(finalSBoxes[i], i, finalTest);
	}
	...
	FILE * triggerLuts = fopen("./triggerLuts.txt", "w");
	
	printf("-----------------------ONE HOT LUTS-----------------------\n");
	fputs("-----------------------ONE HOT LUTS-----------------------", triggerLuts);
	fputc(10, triggerLuts);
	

	for(int i = 0; i < secondLayerIndex; i++){
		printf("%s.%s\n", triggerLutTiles[i], triggerLutNames[i]);
		fprintf(triggerLuts, "%s.%s", triggerLutTiles[i], triggerLutNames[i]);
		fputc(10, triggerLuts);
	}
	...
	fprintf(triggerLuts, "%s.%s", thirdLayerOutputTiles[0], lutName);
